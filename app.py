...
app = Flask(__name__)
app.secret_key = "english_game_final_key_v3"

# =========================
# 단어 데이터 (중복 단어 완벽 제거 및 수정)
# =========================
easy = [
...
hard = [
    ("philosophy", "철학"), ("psychology", "심리학"), ("democracy", "민주주의"), ("globalization", "세계화"),
    ...
    ("mechanism", "메커니즘"), ("complexity", "복잡성"), ("adaptation", "적응"), ("resilience", "회복력"),
    ...
    # 중복 제거된 자리에 들어간 새로운 고급 단어들
    ("aesthetic", "미학적인"), ("bureaucracy", "관료주의"), ("sovereignty", "주권"), ("ambiguity", "모호성")
]
...
def home():
    ...
    if request.method == "POST":
        ...
        if action == "start" and session["step"] == "start":
            ...
            # 무작위 셔플 후 딱 50개만 슬라이싱하므로 '한 판 안에서' 절대 중복되지 않음
            random.shuffle(pool)
            pool = pool[:50] 
